...
class TestSSHIntegration:
    ...
    def _execute_and_wait(
        self, session_manager, ssh_config, command, timeout=10, expected_input=None
    ):
        """
        ...
        if exit_code == 124:
            if stderr.startswith("ASYNC:"):
                ...
                while (
                    time.time() - start_time < timeout + 5
                ):  # Give a little extra time for async completion
                    ...
                    if status_response["status"] != "running":
                        return (
                            status_response["stdout"],
                            status_response["stderr"],
                            status_response["exit_code"] or 0,
                        )
                    time.sleep(0.5)
                ...
                if expected_input:
                    ...
                    return (
                        "",
                        "AWAITING_INPUT command timed out after providing input",
                        124,
                    )
                else:
                    return (
                        stdout,
                        f"Command awaiting input ({reason}) but no expected_input provided.",
                        124,
                    )
    ...
    def test_stderr_output(self, session_manager, ssh_config):
        ...
        assert (
            "error message" in stdout or "error message" in stderr
        )  # Depending on shell, stderr can be merged to stdout
        assert exit_code == 0
    ...
    def test_directory_persistence(self, session_manager, ssh_config):
        ...
        # Cleanup
        self._execute_and_wait(session_manager, ssh_config, command="cd ~", timeout=5)
        self._execute_and_wait(
