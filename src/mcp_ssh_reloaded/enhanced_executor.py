...
class EnhancedCommandExecutor:
    ...
    async def execute_command_enhanced(
        ...
        try:
            # Validate command
            is_valid, error_msg = (
                self.session_manager._command_validator.validate_command(
                    command,
                    pty_aware=(
    ...
    async def _execute_streaming_command(
        ...
        self.context_logger.log_operation_start(
            "streaming_execution", f"timeout={timeout}"
        )
        ...
        try:
            ...
            while True:
                ...
                if shell.recv_ready():
                    ...
                    if running_cmd.progress_callback:
                        ...
                        ).total_seconds() > 5.0:  # Every 5 seconds
                ...
                if elapsed > current_timeout:
                    if (
                        running_cmd.auto_extend_timeout
                        and current_timeout < running_cmd.max_timeout
                    ):
                        # Auto-extend timeout
                        new_timeout = min(
                            current_timeout * 1.5, running_cmd.max_timeout
                        )
                        ...
                        self.context_logger.log_operation_end(
                            "streaming_execution", success=False, details=error_msg
                        )
            ...
            self.context_logger.log_operation_end(
                "streaming_execution", success=False, details=error_info.message
            )
            return ErrorHandler.format_error_for_ai(error_info)
    ...
    async def _execute_enhanced_sync_command(
        ...
        try:
            ...
            if running_cmd.auto_extend_timeout:
                ...
                return self._execute_standard_with_monitoring(
                    command_id, running_cmd, timeout, session_key
                )
            ...
            self.context_logger.log_operation_end(
                "enhanced_sync_execution", success=False, details=error_info.message
            )
            return ErrorHandler.format_error_for_ai(error_info)
        ...
        while True:
            ...
            if elapsed > current_timeout:
                if current_timeout < running_cmd.max_timeout:
                    ...
                    self.context_logger.log_operation_end(
                        "auto_extend_execution", success=False, details=error_msg
                    )
    ...
    def _execute_standard_with_monitoring(
        self,
        command_id: str,
        running_cmd: RunningCommand,
        timeout: int,
        ...
        return (
            stdout
            if exit_code == 0
            else stderr or f"Command failed with exit code {exit_code}"
        )
    ...
    def _monitor_command_progress(self, command_id: str, running_cmd: RunningCommand):
        ...
        while (
            ...
            and not running_cmd.monitoring_cancelled.is_set()
        ):
            now = datetime.now()
    ...
    def _send_progress_update(self, command_id: str, running_cmd: RunningCommand):
        ...
        try:
            ...
            progress_msg = ProgressReporter.format_progress(
                int(duration),
                running_cmd.max_timeout or 300,
                "Command execution",
                f"{output_size} bytes output",
            )
    ...
    def get_command_status_enhanced(self, command_id: str) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            if running_cmd.end_time:
                ...
                status_data["duration_seconds"] = (
                    running_cmd.end_time - running_cmd.start_time
                ).total_seconds()
            ...
            if len(running_cmd.stdout) > 1000:
                ...
                status_data["output_size_display"] = (
                    f"{len(running_cmd.stdout):,} bytes"
                )
            else:
