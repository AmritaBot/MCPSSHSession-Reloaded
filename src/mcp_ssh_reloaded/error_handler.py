...
class ErrorHandler:
    ...
    def _fallback_categorize(
        cls, error_message: str, original_exception: Exception | None
    ) -> ErrorInfo:
        ...
        if any(
            keyword in error_lower for keyword in ["command", "not found", "invalid"]
        ):
    ...
    def _get_user_friendly_message(
        cls, category: ErrorCategory, original_message: str
    ) -> str:
        ...
        if category == ErrorCategory.NETWORK:
            ...
            return (
                "Command execution failed. Check command syntax and execution context."
            )
    ...
    def format_error_for_ai(
        cls, error_info: ErrorInfo, include_troubleshooting: bool = True
    ) -> str:
        ...
        if include_troubleshooting and error_info.troubleshooting_hint:
            response_parts.extend(
                ["", "🔧 Troubleshooting Steps:", error_info.troubleshooting_hint]
            )

        if (
            error_info.original_error
            and error_info.original_error != error_info.message
        ):
            response_parts.extend(
...
class ProgressReporter:
    ...
    @staticmethod
    def format_progress(
        current: int, total: int, operation: str, item: str | None = None
    ) -> str:
    ...
    def format_streaming_output(
        output: str, command_id: str, buffer_size: int = 0
    ) -> str:
