...
class SessionDiagnosticsProvider:
    ...
    def get_session_diagnostics(
        ...
        with self.session_manager._lock:
            ...
            diagnostics.command_history = self._get_recent_commands(
        ...
        try:
            ...
            if shell:
                ...
                if transport:
                    ...
                    state["transport_version"] = getattr(
    ...
    def _get_recent_commands(self, session_key: str, limit: int = 10) -> list[str]:
        ...
        try:
            history = self.session_manager.command_executor.list_command_history(
                limit=50
            )
    ...
    def reset_session_prompt_detection(
        self, host: str, username: str | None = None, port: int | None = None
    ) -> bool:
    ...
    def suggest_session_optimization(self, session_key: str) -> list[str]:
        ...
        try:
            ...
            if shell_type == "unknown":
                suggestions.append(
                    "Shell type not detected - may affect prompt detection"
                )
            ...
            if active_cmd and hasattr(active_cmd, "start_time"):
                ...
                if runtime.total_seconds() > 300:  # 5 minutes
                    suggestions.append(
                        "Command has been running long - consider using streaming mode"
                    )
        ...
        if session_key in self._profiles:
            ...
            if profile.connect_count == 1:
                ...
                profile.avg_connect_time = (
                    profile.avg_connect_time * (profile.connect_count - 1)
                    + connect_time
                ) / profile.connect_count

    def get_performance_report(self) -> dict[str, Any]:
        """Get performance report for connection profiles."""
        report = {"total_profiles": len(self._profiles), "profiles": {}}
        ...
        for session_key, profile in self._profiles.items():
            report["profiles"][session_key] = {
                ...
                "last_connect": profile.last_connect.isoformat()
                if profile.last_connect
                else None,
